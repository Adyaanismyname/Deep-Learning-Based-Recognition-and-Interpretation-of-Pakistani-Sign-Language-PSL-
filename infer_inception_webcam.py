#!/usr/bin/env python3
"""
Webcam inference for the trained InceptionV3 classifier (36 classes).

Behavior:
- Captures frames from webcam
- Uses MediaPipe Hands for robust hand detection and landmark tracking
- If hand is found: crops ROI around hand landmarks, resizes to 299x299, normalizes with ImageNet mean/std
- Loads the full InceptionV3 classifier checkpoint and runs inference (36-way)
- Displays top-3 class names and probabilities

Usage:
python infer_inception_webcam.py --checkpoint /Users/adyaanahmed/Downloads/best_model.pth

Dependencies:
pip install torch torchvision timm opencv-python mediapipe

Notes:
- InceptionV3 uses 299x299 input size (unlike VGG16's 224x224)
- This script assumes checkpoints contain a 'model_state_dict' key
"""
...
def build_model(checkpoint_path, device, num_classes=36):
    """Build InceptionV3 model and load checkpoint."""
    # InceptionV3 from timm
    ...
    if checkpoint_path and Path(checkpoint_path).exists():
        ...
        # Remove 'model.' prefix if present (common in custom training scripts)
...
def preprocess_roi(roi_rgb):
    """Preprocess ROI: resize to 299x299 (InceptionV3 input size), normalize with ImageNet mean/std."""
    # InceptionV3 uses 299x299 input
    img = cv2.resize(roi_rgb, (299, 299), interpolation=cv2.INTER_LINEAR)
...
def main():
    parser = argparse.ArgumentParser(description="InceptionV3 PSL Classifier Webcam Inference")
    ...
    num_classes = len(label_list)  # default
    if args.checkpoint and Path(args.checkpoint).exists():
        try:
            ckpt = torch.load(args.checkpoint, map_location='cpu', weights_only=False)
            
            # Try to get label_map from checkpoint
            ...
            # Detect num_classes from fc layer size in checkpoint
            state_dict = ckpt.get('model_state_dict', ckpt)
            for key in ['model.fc.weight', 'fc.weight']:
                if key in state_dict:
                    detected_classes = state_dict[key].shape[0]
                    if detected_classes != num_classes:
                        print(f"🔍 Detected {detected_classes} classes from checkpoint (fc layer)")
                        num_classes = detected_classes
                        # Expand label list if needed
                        if len(label_list) < num_classes:
                            for i in range(len(label_list), num_classes):
                                label_list.append(f"Class_{i}")
                    break
        except Exception as e:
            print(f"⚠️  Warning reading checkpoint metadata: {e}")

    # build model and load checkpoint
    model = build_model(args.checkpoint, device, num_classes=num_classes)
    ...
    last_bbox = None
    bbox_alpha = 0.6  # smoothing factor for bbox
    ...
    try:
        while True:
            ...
            cv2.imshow('InceptionV3 PSL Inference - Press q to quit', display_frame)
